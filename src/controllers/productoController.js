...
const crearProducto = async (req, res) => {
  ...
  if (errorExistente && errorExistente.code !== 'PGRST116') {
    return res.status(500).json({ mensaje: 'Error al verificar producto existente', error: errorExistente.message });
  }
  ...
  if (error) {
    return res.status(500).json({ mensaje: 'Error al crear el producto', error: error.message });
  }
